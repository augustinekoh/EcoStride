      ...
      <section>
        ...
        <form onSubmit={handleSearch} className="relative flex items-center">
          ...
          <input
            ...
            className="w-full glass-active rounded-2xl pl-12 pr-4 py-3 font-bold text-[var(--color-text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--color-teal-dark)]/50 transition-all border border-black/5 shadow-sm"
          />
          <button 
            type="submit" 
            className="absolute right-2 bg-[var(--color-teal-dark)] text-white p-2 rounded-xl shadow-md hover:scale-105 transition-transform"
          >
            <Search size={16} />
          </button>
        </form>
            ...
            {searchResults.map(user => {
              ...
              return (
                <div 
                  ...
                  className="glass-card rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:bg-white/10 transition-colors"
                >
                  ...
                  {!isAccepted && !isPending && (
                    <button 
                      ...
                      className="glass-active px-3 py-1.5 rounded-full flex items-center gap-1.5 text-xs font-bold text-[var(--color-teal-dark)] hover:bg-[var(--color-teal-dark)] hover:text-white transition-colors"
                    >
          ...
          <div className="space-y-3">
            {friends.filter(f => f.status === 'accepted').map(friend => (
              <div 
                ...
                className="glass-card rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:bg-white/10 hover:-translate-y-0.5 transition-all"
              >
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-[var(--color-teal-dark)]/20 flex items-center justify-center text-[var(--color-teal-dark)] shadow-inner overflow-hidden shrink-0">
                    {friend.avatar ? (
