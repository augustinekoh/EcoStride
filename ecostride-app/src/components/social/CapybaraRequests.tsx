...
export const CapybaraRequests: React.FC = () => {
  ...
  const handleAction = async (mailId: string, action: 'accept' | 'reject') => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/mail/${mailId}/action`, {
        method: 'POST',
  ...
  const handleDismiss = async (mailId: string) => {
    try {
      const token = await auth.currentUser?.getIdToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/mail/user/${mailId}`, {
        method: 'DELETE',
