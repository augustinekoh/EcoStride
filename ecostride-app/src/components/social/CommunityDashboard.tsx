...
import { Trophy, TreeDeciduous, Users, MoreVertical, ShieldAlert, MicOff, Mic, UserMinus, UserIcon, Settings } from 'lucide-react';
    ...
    <div className="w-full pb-24" onClick={() => { setActiveMenuId(null); setMuteMenuId(null); }}>
      ...
      <div className="relative pt-6 pb-8 px-6">
        <div className="flex flex-col items-center text-center">
          <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-tr from-[var(--color-teal-dark)] to-cyan-400 p-1 shadow-xl shadow-[var(--color-teal-dark)]/10 mb-6 relative">
            <div className="w-full h-full bg-white rounded-[1.8rem] flex items-center justify-center relative overflow-hidden">
              <div className="absolute inset-0 bg-[var(--color-teal-dark)]/5 backdrop-blur-xl" />
              {guild?.icon ? (
                <span className="text-4xl relative z-10">{guild.icon}</span>
              ) : (
                <Users size={40} className="text-[var(--color-teal-dark)] relative z-10" />
              )}
            </div>
            <div className="absolute -bottom-2 -right-2 bg-white rounded-full p-2 border border-black/5 shadow-md">
              <TreeDeciduous size={16} className="text-emerald-400" />
            </div>
            {isAdmin && (
              <button 
                onClick={(e) => { e.stopPropagation(); setIsEditingCommunity(true); }}
                className="absolute -top-2 -right-2 bg-white rounded-full p-2 border border-black/5 shadow-md hover:bg-slate-50 transition-colors"
          ...
          <p className="text-slate-500 text-sm max-w-sm mb-6">
            {isLoading ? 'Fetching community details...' : guild?.description || 'Join us in making the world a greener place, one step at a time.'}
          </p>

          <div className="flex gap-4">
            <div className="glass-active bg-white/50 border border-black/5 rounded-2xl px-5 py-3 flex flex-col items-center backdrop-blur-md shadow-sm">
              <span className="text-2xl font-bold text-slate-800 mb-1">{members.length}</span>
              <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Members</span>
            </div>
            <div className="glass-active bg-white/50 border border-black/5 rounded-2xl px-5 py-3 flex flex-col items-center backdrop-blur-md shadow-sm">
              <span className="text-2xl font-bold text-[var(--color-teal-dark)] mb-1">
                {members.reduce((acc, m) => acc + (m.total_trees_planted || 0), 0)}
              </span>
              <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Trees Planted</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-8">
          ...
          {isLoading ? (
            ...
            members.map((member, index) => (
              <div 
                ...
                className={`flex items-center glass-card bg-white/50 border border-black/5 rounded-2xl p-4 shadow-sm cursor-pointer hover:bg-white/80 transition-colors relative ${activeMenuId === member.id ? 'z-50' : 'z-0'}`}
              >
        ...
        <button 
          ...
          className={`w-full mt-8 py-4 glass-active border rounded-2xl font-bold transition-all shadow-sm ${
            (isAdmin && members.length > 1) 
              ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' 
              : isAdmin && members.length === 1
                ? 'bg-red-50 text-red-500 border-red-200 hover:bg-red-100'
                : 'bg-white/50 text-slate-500 border-black/5 hover:bg-red-50 hover:text-red-500 hover:border-red-200'
          }`}
        >
      ...
      {/* The Floating Chat */}
      <FloatingChat guildId={activeGuildId} />
