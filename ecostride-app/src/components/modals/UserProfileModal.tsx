...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose, player }) => {
  ...
  const distanceWalked = typeof distanceRaw === 'number' ? `${distanceRaw}km` : (distanceRaw || '0km');
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
      ...
      <div 
        className="rounded-[2.5rem] w-full max-w-sm max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-90 duration-300 relative border border-white/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]"
        style={{ background: 'linear-gradient(135deg, #fba8d5, #ffb1b1, #ffc9b1, #ffe4b1, #fff4b1)' }}
      >
        ...
        <div className="p-6 flex flex-col items-center gap-5 text-center overflow-y-auto custom-scrollbar">
          ...
          <div className="w-24 h-24 rounded-full border-4 border-white/40 bg-white/20 backdrop-blur-lg flex items-center justify-center text-5xl shadow-lg overflow-hidden shrink-0">
