    ...
    const fetchData = async () => {
      try {
        ...
        const users = userSnap.docs.map(doc => {
          ...
          return {
            email: data.email,
            points: Math.floor(weeklyDist * 50)
          };
        });
        users.sort((a,b) => b.points - a.points);
        setTopUsers(users.slice(0, 3));

        const storeSnap = await getDocs(collection(db, 'point_store'));
        let cats = Array.from(new Set(storeSnap.docs.map(d => d.data().category).filter(Boolean))) as string[];
