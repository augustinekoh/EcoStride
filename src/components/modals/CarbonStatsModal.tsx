...
export const CarbonStatsModal: React.FC<CarbonStatsModalProps> = ({ isOpen, onClose }) => {
  ...
  const weeklyData = days.map((dayLabel, index) => {
    ...
    // Industry average: Walking saves ~0.2kg CO2 per km compared to driving
    const carbon = distance * 0.2;
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-[var(--color-teal-dark)]/20 backdrop-blur-xl flex flex-col items-center justify-end md:justify-center animate-in fade-in duration-300 p-4 md:p-0">
        ...
        <div className="p-6 overflow-y-auto">
          <div className="glass-active rounded-2xl p-5 mb-6 text-center border border-white/50 shadow-sm">
            ...
            <div className="mt-4 bg-[var(--color-pastel-yellow)] text-[var(--color-text-main)] px-4 py-2 rounded-xl text-xs font-bold border border-white/50 inline-block shadow-sm">
              Formula: Total Distance ({calculatedTotalDistance.toFixed(1)} km) × 0.2 kg/km
            </div>
          </div>
