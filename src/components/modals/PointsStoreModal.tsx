...
export const PointsStoreModal: React.FC<PointsStoreModalProps> = ({ onClose, merchantFilter }) => {
  ...
  const [merchants, setMerchants] = useState<{ [key: string]: string }>({});
  ...
  useEffect(() => {
    ...
    const unsubMerchants = onSnapshot(collection(db, 'merchants'), (snapshot) => {
      ...
      snapshot.forEach(doc => {
        merchantMap[doc.id] = doc.data().storeName;
      });
      setMerchants(merchantMap);
    });
    return () => { unsub(); unsubMerchants(); };
  }, []);
  ...
  const handleBuy = async (item: any) => {
    ...
    setBuyingId(item.id);
    try {
      // 1. Deduct coins locally and in user document (assuming deductCoins does both, wait, deductCoins only updates local store in current implementation. Wait, useUserStore should update DB?)
      // We will deduct coins via store, and manually update user doc just to be safe if useUserStore doesn't do it.
      // Actually useUserStore handles DB update if we implemented it right, let's just use it and manually update to be safe.
      ...
      // 3. Record purchase
      const purchaseId = `purchase-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
      ...
      <div className="relative w-full max-w-2xl bg-[#faf9f6] h-[90vh] sm:h-auto sm:max-h-[85vh] rounded-t-3xl sm:rounded-3xl border-t-2 sm:border-2 border-slate-900 shadow-comic flex flex-col animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-8 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-dots">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {storeItems
              .filter(item => merchantFilter ? item.merchantId === merchantFilter : true)
              ...
              .filter(item => {
                ...
                const matchMerchant = item.merchantId && merchants[item.merchantId]?.toLowerCase().includes(searchLower);
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <h3 className="font-black text-xl text-slate-900 mb-1 leading-tight">{item.name}</h3>
                    
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {item.category && <span className="text-[10px] uppercase font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">{item.category}</span>}
                      {item.merchantId && merchants[item.merchantId] && (
                        <span className="text-[10px] uppercase font-bold text-brand-blue bg-brand-blue/10 px-2 py-0.5 rounded-full border border-brand-blue/20">
