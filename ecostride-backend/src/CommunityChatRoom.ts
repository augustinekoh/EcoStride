...
export class CommunityChatRoom extends DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (!userId || !guildId) {
      return new Response('Missing userId or guildId', { status: 400 });
    }
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer) {
    ...
    try {
      ...
      if (data.type === 'message') {
        ...
        // @ts-ignore
        const userCheck = await this.env.DB.prepare('SELECT username, muted_until FROM users WHERE id = ?').bind(userId).first();
        if (userCheck) {
        ...
        // @ts-ignore
        this.env.DB.prepare(
          "INSERT INTO chat_messages (id, guild_id, user_id, content, created_at) VALUES (?, ?, ?, ?, ?)"
        ).bind(msgId, guildId, userId, content, createdAt).run().catch((err: any) => {
